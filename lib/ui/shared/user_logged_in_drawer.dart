...
class UserLoggedInDrawer extends StatefulWidget {
  UserLoggedInDrawer({
    this.logoutAction,
  ...
  _UserLoggedInDrawerState createState() => _UserLoggedInDrawerState();
}

class _UserLoggedInDrawerState extends State<UserLoggedInDrawer> {
              ...
              ListTile(
                onTap: () {
                  ...
                  Navigator.of(SharedContexts.homeContext)
                      .push(MaterialPageRoute(builder: (_) => UserAddOffer(
                        isAddOffer: true,
                      )));
                ...
                title: Text(
                  ArabDealLocalization.of(context)
                      .getTranslatedWordByKey(key: 'drawer_page_add_offer'),
                ),
                leading: Icon(Icons.add, color: Colors.grey[700]),
              ),
              (sharedPreferences.getBool('getNotifications'))
                  ? ListTile(
                      onTap: () {
                        Navigator.of(context).pop();
                      ...
                      title: Text(
                        ArabDealLocalization.of(context).getTranslatedWordByKey(
                            key: 'all_drawers_favorie_categories'),
                      ),
              ...
              ListTile(
                onTap: () {
                  ...
                  Navigator.of(widget.currentContetxt)
                      .popUntil((route) => route.isFirst);
                  Navigator.of(SharedContexts.homeContext)
                      .push(MaterialPageRoute(builder: (_) => EditAccont()));

                  //TODO POP UNTIL HOME PAGE AND THEN GO TO CATEGORIES PAGE
                },
                title: Text(
                  ArabDealLocalization.of(context).getTranslatedWordByKey(
                      key: 'edit_account_page_edit_account'),
                ),
                leading: Container(
                    child: CachedImageFromNetwork(urlImage: user.imageUrl),
                    width: 30,
                    height: 30),
